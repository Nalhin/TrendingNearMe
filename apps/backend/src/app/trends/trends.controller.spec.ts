...
import { of } from 'rxjs';
import {
  coordinatesDtoFactory, trendHistoryDetailsResponseDtoFactory,
  trendHistoryResponseDtoFactory,
...
describe('Trends Controller', () => {
  ...
  describe('getTrendsByLocation', () => {
    it('should return correct response type', (done) => {
      ...
      jest.spyOn(trendsService, 'getTrendsByLocation').mockReturnValue(of(response));
  ...
  describe('getHistory', () => {
    it('should return correct response type', (done) => {
      ...
      const response = trendHistoryResponseDtoFactory.buildMany(3);
      jest.spyOn(trendsService, 'getHistory').mockReturnValue(of(response as any));
  ...
  describe('getHistoryById', () => {
    it('should return correct response type', async () => {
      const user = appUserFactory.buildOne();
      const response = trendHistoryDetailsResponseDtoFactory.buildOne();
      jest.spyOn(trendsService, 'getHistoryById').mockResolvedValueOnce(response as any);

      const result = await controller.getHistoryById({ id: response._id }, user);
