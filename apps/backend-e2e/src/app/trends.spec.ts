...
import { trendDocumentFactory } from '../../../backend/test/fixtures/trends.fixture';
import { Trend } from '../../../backend/src/app/trends/trends.schema';
import { MongooseTestUtils } from '../utils/mongoose.test-utils';
import * as request from 'supertest';
import { appUserFactory, userDocumentFactory } from '../../../backend/test/fixtures/users.fixture';
import { authHeaderFactory } from '../utils/auth.utils';
import { AppUser, User, UserDocument } from '../../../backend/src/app/users/users.schema';
import { of } from 'rxjs';
import { twitterClosestResponse, twitterTrendResponse } from '../../../backend/test/fixtures/twitter-response.fixture';
...
describe('TrendsController (e2e)', () => {
  ...
  describe('/history (GET)', () => {
    ...
    beforeEach(async () => {
      ...
      await testUtils.saveMany(Trend, trendDocumentFactory.buildMany(3).map(u => ({ ...u, user })));
  ...
  describe('/history/:id (GET)', () => {
    ...
    it('should return search history details', async () => {
      const trendData = trendDocumentFactory.buildOne({ user: user as AppUser });
    ...
    it('should return error if user is not authorized', async () => {
      const trend = await testUtils.save(Trend, trendDocumentFactory.buildOne());
